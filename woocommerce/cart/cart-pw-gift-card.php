...
if ( isset( $session_data['gift_cards'] ) ) {
    ...
    foreach ( $session_data['gift_cards'] as $card_number => $discount_amount ) {
        ...
        if ( $pw_gift_card->get_id() ) {
            ...
            <tr class="pwgc-total">
                <th>
                    <?php _e( 'Gift card', 'levre' ); ?>
                    <div class="pwgc-cart-subtitle">
                        <?php echo FSD_Theme::check_for_empty($pw_gift_card->get_number()); ?><br />
                        <?php echo sprintf( __( 'Remaining balance is %s', 'levre' ), wc_price( $balance ) ); ?>
                        <?php
                            if ( $pw_gift_card->has_expired() ) {
                                ?>
                                <br />
                                <span>
                                    <?php _e( 'Expired', 'levre' ); ?>
                                </span>
                                <?php
                            }
                        ?>
                ...
                <td data-title="<?php esc_attr_e( 'Gift Card Total', 'levre' ); ?>">
                    <?php echo wc_price( $discount_amount * -1 ); ?>
                    <a href="#" class="pwgc-remove-card" data-card-number="<?php esc_attr( $card_number ); ?>"><?php _e( '[Remove]', 'levre' ); ?></a>
                </td>
            </tr>
            <?php
        }
